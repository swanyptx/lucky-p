...
const BasicSection = ({ title, text, image, reverse, sectionName, button, buttonContent = "", href = "/", fade }) => {
    ...
    return (
        <section id={`${section}`} className={`${section} flex justify-center items-center lg:py-20 py-10`}>
            <div data-aos={`fade-${fade}`} className={reverse ? `container sectionDivReverse flex justify-between items-center lg:py-8 ` : `container sectionDiv flex justify-between items-center lg:py-8 grid lg:grid-cols-2 sm:grid-cols-1`}>
